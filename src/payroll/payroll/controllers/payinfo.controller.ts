...
export class PayInfoController {
  ...
  async create(@Body() data:PayInfo){
    return new TestResponse("Created payment record", await this.payinfoService.create(data));
    }


  @Get()
  async findAll() {
    return new TestResponse("Fetched all payment records", await this.payinfoService.findAll());
  }

  @Get(':id')
  async findOne (@Param('id') id:ObjectId){
    return new TestResponse("Fetched single payment record", await this.payinfoService.findOne(id));
  }
  
  @Patch(':id')
  async update(@Param('id') id: ObjectId, @Body() data:PayInfo,@getUser()user) {
    return new TestResponse("Updated payment record successfully", await this.payinfoService.update(id, data,user));
  } 


   @Delete(':id')
    async delete(@Param('id') id:ObjectId){
      return new TestResponse("Deleted payment record", await this.payinfoService.delete(id));
    }
