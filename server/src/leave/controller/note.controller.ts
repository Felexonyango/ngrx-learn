...
export class NoteController {
    ...
    async notesForALeave(@Param('leaveId') leaveId:ObjectId){
        return new SibasiResponse('Notes for leave successfully fetched', await this.noteService.getLeaveNotes(leaveId));
    }
    ...
    async postNoteForLeave(@Body() data:NoteDTO, @Param('leaveId') leaveId:ObjectId, @getUser() user){
        ...
        return new SibasiResponse('Note successfully posted', await this.noteService.postNote(data));
    }

    @Delete('/:leaveId/:noteId')
    async deleteNote(@Param('leaveId') leaveId:ObjectId, @Param('noteId') noteId:ObjectId, @getUser() user){
        return new SibasiResponse('Note successfully deleted', await this.noteService.deleteNote(leaveId, noteId, user.userId));
    }
}