...
export class CompanyController {
  ...
  async create(@Body() company:Company){
    return new TestResponse('Created company successfully', await this.companyService.create(company));        
    }
  ...
  async postLogo(@UploadedFile() file, @Param() companyId:ObjectId,@getUser() user){
    upload.single('file');
    return new TestResponse('', await this.uploadsService.uploadFile(companyId,file,user));
  }
 
  @Get()
  async findAll() {
    return new TestResponse("Fetched all company records", await this.companyService.findAll());
  }

  @Get(':id')
  async findOne (@Param('id') id:ObjectId){
    return new TestResponse('Fetched records for a single company', await this.companyService.findOne(id));
  }
  
  @Patch(':id')
  async update(@Param('id') id: ObjectId, @Body() data:Company) {
    return new TestResponse('Updated record for a company', await this.companyService.update(id, data));
  } 


   @Delete(':id')
    async delete(@Param('id') id:ObjectId){
      return new TestResponse('Deleted company records successfully', await this.companyService.delete(id));
    }
